...
`/prospecting` → **リスト管理** タブ →「今日の追記」を開いて、ボタン2つ。

1. **「Claude で調べる」** を押す → 調査プロンプトが入った状態で Claude が新しいタブで開く。**送信を押すだけ**
   - 曜日ごとのテーマ（月＝Java／火＝インフラ／水＝パートナー募集／木＝フロント／金＝資金調達／土＝PHP・モバイル・地方／日＝設備系IT）が自動で入る
   - 会社のPCで claude.ai を開けない場合は「プロンプトだけコピー」を押して、使える Claude に貼る
2. Claude の**回答をまるごとコピー**（コードブロックだけ選ぶ必要なし）
3. 画面に戻って **「クリップボードから取り込む」** を押す → その場で **追加◯社／スキップ◯社**（内訳つき）が出る
   - 前置きの文章や「以上です」といった後書きは自動で捨て、表の部分だけ取り込む
   - クリップボードの読み取りをブラウザが拒否した場合は、「手で貼り付ける」を開いて貼り付け →「リストに追記」
4. 直近7日の棒グラフで積み上がりを確認する
