...
/**
 * DBに残っている旧ステージ名。`normalizeStage()` が新名へ寄せる対象と同じ一覧。
 *
 * 移行前のデータが `提案済`/`返信待ち` などのまま保存されているため、
 * **stage を IN で絞る箇所では新名だけでなくこの旧名も含めないと行が消える**。
 */
export const LEGACY_STAGES = ["提案済", "返信待ち", "返信あり", "面談調整", "クロージング中", "面談合格"] as const;

/**
 * 「進行中」として数える stage の正。新6ステージ＋DBに残る旧名。
 *
 * ここを共通化した理由（2026-07）：同じ本番アプリの中で3種類の一覧が使われており、
 * 画面ごとに件数が食い違っていた。
 *   - 提案管理 `/proposals` … 新6種のみ → **旧名の提案が一覧から消えていた**
 *   - ダッシュボード・分析   … 旧名は含むが `承認待ち`/`確認中` が抜けていた
 *     → 確認中の提案がパイプラインから消えていた
 * DBのステージ名を正規化（`supabase/proposal-stage-normalize.sql`）したあとは
 * LEGACY_STAGES を外して新6種だけにできる。
 */
export const ACTIVE_STAGES: string[] = [...PROPOSAL_STAGES, ...LEGACY_STAGES];
...
export const LOST_REASONS = [
  ...
  "E1: 担当者と連絡つかず", "E2: タイミング逃した", "E3: その他", "E4: 連絡取れたが進展なし", "架電できていない",
];
