...
export function dailyResearchPrompt(theme: DailyTheme, opts: { date?: string; target?: number; angle?: string; avoid?: string[] } = {}): string {
  ...
  // トークン節約：説明文を書かせず CSV だけ返させる（出力トークンが数分の一になる）。
  //   ルールは1行1件の短文にまとめ、書式の見本行は載せない
  //   （見本をそのままコピーして「株式会社サンプル」が登録される事故を防ぐ）。
  return [
    `IT人材営業（SES・業務委託・紹介）のリサーチです。日本国内で「いまエンジニアを募集している企業」を Web検索で${target}社集め、CSVにしてください。`,
    "",
    `# テーマ（${date} ${theme.weekday}曜）`,
    theme.detail,
    ...extra,
    "",
    "# ルール",
    "- 採用ページ・求人・SES/パートナー募集で、エンジニア募集中と確認できた企業だけ",
    "- URLは実際に開けたものだけ（推測で作らない）。確認は各社1〜2ページで十分",
    "- 「営業お断り」の記載がある企業は除外",
    "- 求人媒体（Green・Wantedly等）は発見にのみ使い、URLは企業の公式サイト・採用ページを載せる",
    "- 個人名・個人の連絡先は扱わない",
    `- ${target}社に届かない場合は、確認できた分だけでよい`,
    "",
    "# 出力（重要）",
    "説明・前置き・後書きは書かず、CSVのコードブロックだけを出力してください。1行1社。",
    "企業名,採用ページURL,企業URL,業種,所在地,ランク,シグナル,発見元,メモ",
    "- ランク：A=今週送る B=来週以降 C=対象外",
    `- シグナル：${PROSPECT_SIGNALS.join(" / ")} から該当のみ「;」区切り（無ければ空欄）`,
    "- 発見元：媒体名（PR TIMES／Green／企業HP など）",
    "- メモ：募集職種を10字程度（例：Java中途3名）",
    "- 不明な列は空欄。カンマを含む値は\"で囲む。セル内改行なし",
  ].join("\n");
}
