...
export default function LoginPage({ searchParams }: { searchParams: Promise<{ redirect?: string; err?: string; confirmed?: string; reset?: string }> }) {
  const { redirect = "/", err, confirmed, reset } = use(searchParams);
  ...
  // ?reset=done は #454：enger.jp 側でパスワードを設定し終えたエージェントの着地（新PWでログイン促し）。
  const notice = !error
    ? (reset === "done"
      ? "パスワードを設定しました。新しいパスワードでログインしてください。"
      : confirmed === "1" ? "メールアドレスの確認が完了しました。管理者の承認後にログインできます。" : null)
    : null;
        ...
        <section style={{ display: "grid", placeItems: "center", padding: 24 }}>
          <div style={{ width: "100%", maxWidth: 380 }}>
            ...
            <form action={action} style={{ background: "rgba(255,255,255,.97)", borderRadius: 18, padding: 30, display: "flex", flexDirection: "column", gap: 13, boxShadow: "0 24px 70px rgba(0,0,0,.35)", backdropFilter: "blur(6px)" }}>
              ...
              <a
                href={`/api/auth/google?next=${encodeURIComponent(redirect)}`}
              ...
              {/* #309②：GitHub でもログイン/登録できる（初回は承認待ちの法人アカウントを作成）。 */}
              <a
                href={`/api/auth/github?next=${encodeURIComponent(redirect)}`}
                style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "11px", border: "1px solid #24292f", borderRadius: 10, background: "#24292f", color: "#fff", fontSize: 14, fontWeight: 600, textDecoration: "none" }}
              >
                <svg width="18" height="18" viewBox="0 0 16 16" fill="#fff" aria-hidden><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0016 8c0-4.42-3.58-8-8-8z"/></svg>
                GitHub でログイン
              </a>
              ...
              {/* ログイン先の分離：こちらは法人・社内（ENGER business）専用。フリーランス（個人）は enger.jp へ誘導。 */}
              <div style={{ marginTop: 4, fontSize: 11.5, color: "#6b7280", textAlign: "center", lineHeight: 1.8, background: "#f8fafc", border: "1px solid #eef2f7", borderRadius: 10, padding: "9px 10px" }}>
                こちらは <b style={{ color: "#0F2440" }}>ENGER business（企業・社内）</b> 用のログインです。<br />
                フリーランス（個人）の方は <a href="https://enger.jp" style={{ color: "#0095D9", fontWeight: 700, textDecoration: "none" }}>ENGERフリーランス（enger.jp）</a> からログインしてください。
              </div>
            </form>
          </div>
        </section>
