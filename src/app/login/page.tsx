        ...
        <section style={{ display: "grid", placeItems: "center", padding: 24 }}>
          <div style={{ width: "100%", maxWidth: 380 }}>
            ...
            <form action={action} style={{ background: "rgba(255,255,255,.97)", borderRadius: 18, padding: 30, display: "flex", flexDirection: "column", gap: 13, boxShadow: "0 24px 70px rgba(0,0,0,.35)", backdropFilter: "blur(6px)" }}>
              ...
              {/* 利用規約・プライバシー同意（ログイン手段の直前） */}
              <label style={{ display: "flex", alignItems: "flex-start", gap: 9, fontSize: 12, color: "#6b7280", lineHeight: 1.6, background: "#f5f8fc", border: "1px solid #e5e9f0", borderRadius: 10, padding: "11px 13px", cursor: "pointer" }}>
                <input type="checkbox" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} style={{ width: 16, height: 16, marginTop: 1, accentColor: "#0095D9", flex: "0 0 auto", cursor: "pointer" }} />
                <span>
                  <a href="/terms" target="_blank" rel="noreferrer" style={{ color: "#0095D9", fontWeight: 600, textDecoration: "underline" }}>利用規約</a>・<a href="/privacy" target="_blank" rel="noreferrer" style={{ color: "#0095D9", fontWeight: 600, textDecoration: "underline" }}>プライバシーポリシー</a>に同意します
                </span>
              </label>

              {/* Google ログイン */}
              <a href="/api/auth/google" aria-disabled={!agreed} onClick={(e) => { if (!agreed) e.preventDefault(); }} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "11px", border: "1px solid #d6dce5", borderRadius: 10, background: "#fff", color: "#3c4043", fontSize: 14, fontWeight: 600, textDecoration: "none", opacity: agreed ? 1 : 0.5, pointerEvents: agreed ? "auto" : "none", cursor: agreed ? "pointer" : "not-allowed" }}>
              ...
              <button type="submit" disabled={pending || !agreed} style={{ background: "linear-gradient(135deg, #0095D9, #007DB3)", color: "#fff", border: 0, borderRadius: 10, padding: "13px", fontSize: 14.5, fontWeight: 700, cursor: pending || !agreed ? "not-allowed" : "pointer", opacity: pending || !agreed ? 0.6 : 1, boxShadow: "0 6px 16px rgba(0,149,217,.3)" }}>{pending ? "ログイン中…" : "メールでログイン →"}</button>
            </form>
            <div style={{ textAlign: "center", fontSize: 12, color: "rgba(255,255,255,.8)", marginTop: 14 }}>アカウントをお持ちでない方は <a href="/signup" style={{ color: "#7dd3fc", fontWeight: 700, textDecoration: "none" }}>新規登録</a></div>
          </div>
        </section>
      </div>
    </div>
