        ...
        <section style={{ display: "grid", placeItems: "center", padding: 24 }}>
          <div style={{ width: "100%", maxWidth: 380 }}>
            ...
            <form action={action} style={{ background: "rgba(255,255,255,.97)", borderRadius: 18, padding: 30, display: "flex", flexDirection: "column", gap: 13, boxShadow: "0 24px 70px rgba(0,0,0,.35)", backdropFilter: "blur(6px)" }}>
              ...
              {/* ログイン方法（同意するまで無効）。同意チェックは下にまとめて配置 */}
              {!agree && <div style={{ fontSize: 12, color: "#b45309", background: "#fff7e6", border: "1px solid #f6d9a7", borderRadius: 8, padding: "8px 11px" }}>下の利用規約に同意すると、ログインが有効になります。</div>}

              {/* Google ログイン */}
              <a
                href="/api/auth/google"
                onClick={(e) => { if (!agree) { e.preventDefault(); setNeedAgree(true); } }}
                aria-disabled={!agree}
                style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "11px", border: "1px solid #d6dce5", borderRadius: 10, background: "#fff", color: "#3c4043", fontSize: 14, fontWeight: 600, textDecoration: "none", opacity: agree ? 1 : 0.5, cursor: agree ? "pointer" : "not-allowed", pointerEvents: agree ? "auto" : "none" }}
              >
              ...
              <label style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 12.5, color: "#475569", fontWeight: 600 }}>メールアドレス
                <input name="email" type="email" required autoComplete="username" placeholder="you@example.com" disabled={!agree} style={{ ...input, opacity: agree ? 1 : 0.55 }} />
              </label>
              <label style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 12.5, color: "#475569", fontWeight: 600 }}>パスワード
                <input name="password" type="password" required autoComplete="current-password" placeholder="••••••••" disabled={!agree} style={{ ...input, opacity: agree ? 1 : 0.55 }} />
              </label>
              ...
              <div style={{ borderTop: "1px solid #eef2f7", marginTop: 4, paddingTop: 12 }}>
                <label style={{ display: "flex", gap: 8, alignItems: "flex-start", fontSize: 12, color: "#4b5563", lineHeight: 1.7, cursor: "pointer" }}>
                  <input type="checkbox" checked={agree} onChange={(e) => { setAgree(e.target.checked); if (e.target.checked) setNeedAgree(false); }} style={{ marginTop: 2, width: 16, height: 16, accentColor: "#0095D9", flex: "0 0 16px" }} />
                ...
                {needAgree && <div style={{ fontSize: 12, color: "#d23f57", marginTop: 6 }}>ログインするには、上のチェックを入れて同意してください。</div>}
              </div>
            </form>
