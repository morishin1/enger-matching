...
export async function POST(req: Request) {
  ...
  const jobListText = jobs.map((j, i) => {
    return `${i + 1}. no=${j.job_no} / ${j.title ?? ""} / ${j.client_name ?? ""} / 職種:${j.role_label ?? "—"} / 単価:${j.salary_min ?? ""}〜${j.salary_max ?? ""}万円 / リモート:${j.remote_type ?? "—"} / 必要スキル:${(j.skills ?? []).join(" ")}`;
  }).join("\n");

  const prompt = [
    "次の人材に対して、各案件の適合度を0〜100で採点し、JSON配列だけを出力してください（説明やコードフェンス不要）。",
    "スキルの類似(表記揺れ・関連技術)、単価整合、職種・リモート条件、経験年数の妥当性を総合評価してください。",
    "重要: 人材の希望単価が案件予算を10万円超で上回る場合は現場で調整できずミスマッチ。スキルが完全一致でも、",
    ...
    '形式: [{"job_no": 数値, "score": 0-100, "reason": "30字程度の根拠"}]',
    "",
    "── 人材 ──",
    candDesc,
    "",
