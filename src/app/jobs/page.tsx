...
async function fetchFocusJob(focus?: string | null): Promise<any | null> {
  ...
  try {
    ...
    const fetchWith = (c: string) => isUuid
      ? sb.from("jobs").select(c).eq("id", v).maybeSingle()
...
//   ・#447②：文言変更に伴い新文言も検索対象に追加（旧文言のデータと新文言で保存された新規データの両方を拾う）。
const FLOW_CAT_TO_LABELS: Record<string, string[]> = {
  jp_to_self:          ["貴社まで"],
  jp_to_self_seishain: ["貴社正社員まで", "貴社社員まで"],
  jp_to_1:             ["貴社一社まで", "貴社一社先まで"],
  jp_to_1_seishain:    ["貴社一社正社員まで", "貴社一社先正社員まで", "貴社一社先社員まで"],
  jp_to_2:             ["貴社二社まで", "貴社二社先まで"],
  jp_to_2_seishain:    ["貴社二社正社員まで", "貴社二社先社員まで"],
  any:                 ["商流不問"],
};
