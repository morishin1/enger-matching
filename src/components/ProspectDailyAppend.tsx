    ...
    <section style={{ ...card, padding: 18 }}>
      ...
      <div style={{ display: "grid", gap: 12, marginTop: 14 }}>
        <div style={{ background: "#f8fafc", border: "1px solid #eaecf0", borderRadius: 14, padding: 13 }}>
          <div style={stepLabel}><Num n={1} />調査プロンプトをコピーして、手元の Claude に貼る</div>
          <p style={{ margin: "6px 0 8px", fontSize: 11.5, color: "#667085", lineHeight: 1.7 }}>
            Claude が Web 検索で今日のテーマの候補企業を調べ、そのまま取り込める CSV を返します。氏名・個人連絡先はプロンプトにも出力にも含めません。
          </p>
          <CopyButton text={prompt} label="調査プロンプトをコピー" />
        ...
        <form action={action} style={{ background: "#f8fafc", border: "1px solid #eaecf0", borderRadius: 14, padding: 13, display: "grid", gap: 8 }}>
          <div style={stepLabel}><Num n={2} />返ってきた CSV を貼って取り込む</div>
          <p style={{ margin: 0, fontSize: 11.5, color: "#667085" }}>
            企業名,採用ページURL,企業URL,業種,所在地,ランク,シグナル,発見元,メモ（従来形式・Excel のタブ区切り貼り付けも自動判別）
          </p>
          <textarea name="csv" rows={7} style={{ ...input, fontSize: 12 }} placeholder={"企業名,採用ページURL,企業URL,業種,所在地,ランク,シグナル,発見元,メモ\n株式会社サンプル,https://example.com/recruit,https://example.com,SaaS,東京都渋谷区,A,資金調達;複数職種募集,PR TIMES,シリーズB調達直後"} />
