    ...
    return items
      .filter((p) => {
        if (q.trim() && !(p.candidate_name ?? "").includes(q.trim())
          ...
          && !(p.closer ?? "").includes(q.trim())) return false;
  ...
  return (
    <div className="card flush">
      ...
      <div style={{ padding: "10px 16px", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <input placeholder="人材・企業・案件・提案者で検索…" value={q} onChange={(e) => setQ(e.target.value)}
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 12.5, minWidth: 880 }}>
            <thead>
              <tr style={{ color: "var(--color-ink-4)", fontSize: 11, background: "var(--color-surface-soft)" }}>
                ...
                <th style={{ padding: "8px 10px", textAlign: "right", width: 90 }}></th>
              </tr>
            </thead>
            <tbody>
