...
function InvCell({ e, period, onChanged }: { e: Eng; period: string; onChanged: () => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 3, minWidth: 150 }}>
      <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
        <input type="number" value={amount} disabled={pending} placeholder="請求額" style={{ ...inp, width: 70 }} onChange={(ev) => setAmount(ev.target.value === "" ? "" : Number(ev.target.value))} onBlur={() => saveBill({ invoice_amount: amount === "" ? null : Number(amount) })} />
...
function Row({ e, role, period, onChanged }: { e: Eng; role: Role; period: string; onChanged: () => void }) {
  const [pending, start] = useTransition();
  ...
  return (
    <tr style={{ opacity: pending ? 0.6 : 1 }}>
      <td style={td}>
        <div style={{ fontWeight: 700, fontSize: 12.5 }}>{e.candidate_name ?? "—"}</div>
      ...
      <td style={td}><input type="number" defaultValue={e.monthly_rate ?? ""} style={{ ...inp, width: 58 }} disabled={pending} onBlur={(ev) => { if (String(ev.target.value) !== String(e.monthly_rate ?? "")) save({ monthly_rate: ev.target.value === "" ? null : Number(ev.target.value) }); }} /></td>
      <td style={td}>{masked ? <Locked /> : <input type="number" defaultValue={e.cost ?? ""} placeholder="—" style={{ ...inp, width: 58 }} disabled={pending} onBlur={(ev) => { if (String(ev.target.value) !== String(e.cost ?? "")) save({ cost: ev.target.value === "" ? null : Number(ev.target.value) }); }} />}</td>
...
export function Workbench({ rows, role = "admin", period, canManage, agentScoped }: { rows: any[]; role?: Role; period: string; canManage: boolean; agentScoped?: boolean }) {
  ...
  const filtered = useMemo(() => rows.filter((e) => !q.trim() || (e.candidate_name ?? "").includes(q.trim()) || (e.company ?? "").includes(q.trim()) || (e.job_title ?? "").includes(q.trim())), [rows, q]);

  const tabBtn = (id: "list" | "card", label: string) => (
  ...
  return (
    <>
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 12 }}>
        <div style={{ display: "flex", gap: 4, padding: 3, background: "var(--color-surface-inset)", borderRadius: 99 }}>{tabBtn("list", "📋 リスト")}{tabBtn("card", "🗂 カード")}</div>
        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
        ...
        <div className="tbl-search" style={{ width: 200, flex: "0 0 200px" }}><input placeholder="人材・企業で検索…" value={q} onChange={(e) => setQ(e.target.value)} /></div>
        {agentScoped && <span className="muted" style={{ fontSize: 11 }}>自分の担当のみ表示</span>}
        <div style={{ marginLeft: "auto" }}>{canManage && <EngagementTools rows={rows} />}</div>
      </div>

      {filtered.length === 0 ? (
        <div className="card" style={{ textAlign: "center", color: "var(--color-ink-4)", padding: 40 }}>対象の稼働がありません。提案管理で成約 →「稼働化」、または「＋新規追加」で登録してください。</div>
      ) : view === "list" ? (
        <div className="card flush" style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 980 }}>
            <thead>
              <tr>
                <th style={th}>人材 / 企業・案件</th><th style={th}>区分</th><th style={th}>月額</th><th style={th}>原価</th><th style={th}>粗利</th><th style={th}>状態</th>
                <th style={th}>当月勤怠({period})</th><th style={th}>契約書/注文書</th><th style={th}>請求({period})</th><th style={th}>満了日</th>
              </tr>
            </thead>
            <tbody>{filtered.map((e) => <Row key={e.id} e={e} role={role} period={period} onChanged={onChanged} />)}</tbody>
          </table>
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 14 }}>
          {filtered.map((e) => <Card key={e.id} e={e} role={role} period={period} onChanged={onChanged} />)}
        </div>
      )}
    </>
  );
...
function Card({ e, role, period, onChanged }: { e: Eng; role: Role; period: string; onChanged: () => void }) {
  const [pending, start] = useTransition();
  const isAdmin = role === "admin";
  const masked = e._maskMargin;
  const rate = Number(e.monthly_rate), cost = Number(e.cost);
  ...
  const setStatus = (s: string) => start(async () => { await updateEngagementStatus(e.id, s); onChanged(); });
  return (
    <div className="card" style={{ display: "flex", flexDirection: "column", gap: 10, padding: 16, opacity: pending ? 0.6 : 1 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontWeight: 700, fontSize: 13.5 }}>{e.candidate_name ?? "—"}</div>
