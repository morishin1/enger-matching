...
import { addProspect, importProspectsCsv, recordProspectActivity, promoteProspectToCompany, updateProspectStatus } from "@/app/prospecting/actions";
...
const importProspectsCsvFormAction = importProspectsCsv as unknown as FormAction;
...
export async function ProspectingPanel({ searchParams, basePath = "/meetings" }: { searchParams: { prospectingTab?: string; owner?: string; source?: string; status?: string }; basePath?: string }) {
  const sp = searchParams;
  const tab = sp.prospectingTab === "list" || sp.prospectingTab === "results" ? sp.prospectingTab : "today";
  ...
  return (
    <div style={{ minHeight: "calc(100vh - 80px)", margin: 0, padding: "24px", background: `linear-gradient(160deg, ${NAVY} 0%, #0a1830 36%, #f6f8fb 36%)` }}>
      <div style={{ maxWidth: 1240, margin: "0 auto" }}>
        ...
        <nav style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
          <Tab href={`${basePath}?section=prospecting`} active={tab === "today"}>今日のアタック</Tab>
          <Tab href={`${basePath}?section=prospecting&prospectingTab=list`} active={tab === "list"}>リスト管理</Tab>
          <Tab href={`${basePath}?section=prospecting&prospectingTab=results`} active={tab === "results"}>成果</Tab>
        </nav>
...
function ListTab({ prospects }: { prospects: Prospect[] }) {
  return <div style={{ display: "grid", gridTemplateColumns: "minmax(280px, 380px) 1fr", gap: 14, alignItems: "start" }}>
    <div style={{ display: "grid", gap: 12 }}><AddForm /><CsvForm /></div>
    <div style={{ ...card, padding: 16, overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}><thead><tr>{["会社", "状態", "優先", "担当", "出所", "操作"].map((h) => <th key={h} style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #eaecf0" }}>{h}</th>)}</tr></thead><tbody>{prospects.map((p) => <tr key={p.id}><td style={{ padding: 8, borderBottom: "1px solid #f2f4f7" }}><b>{p.company_name}</b><div style={{ color: "#667085" }}>{p.website}</div></td><td><Status status={p.status} /></td><td>{p.priority}</td><td>{p.owner_staff}</td><td>{p.source_list}</td><td><form action={updateProspectStatusFormAction}><input type="hidden" name="prospect_id" value={p.id} /><select name="status" defaultValue={p.status} style={input}>{PROSPECT_STATUSES.map((s) => <option key={s}>{s}</option>)}</select><button style={{ ...btn, marginTop: 6 }}>更新</button></form></td></tr>)}</tbody></table></div>
  </div>;
}
function AddForm() { return <form action={addProspectFormAction} style={{ ...card, padding: 16, display: "grid", gap: 9 }}><h2 style={{ margin: 0, fontSize: 16 }}>手入力</h2><input name="company_name" placeholder="会社名*" required style={input} /><input name="industry" placeholder="業界" style={input} /><input name="website" placeholder="URL" style={input} /><input name="contact_form_url" placeholder="問い合わせフォームURL" style={input} /><input name="phone" placeholder="電話" style={input} /><input name="contact_name" placeholder="担当者" style={input} /><input name="owner_staff" placeholder="自社担当" style={input} /><input name="source_list" placeholder="出所（リスト名）" style={input} /><input name="priority" type="number" min="0" max="100" defaultValue="50" style={input} /><textarea name="note" placeholder="メモ" rows={3} style={input} /><button style={btn}>追加</button></form>; }
function CsvForm() { return <form action={importProspectsCsvFormAction} style={{ ...card, padding: 16, display: "grid", gap: 9 }}><h2 style={{ margin: 0, fontSize: 16 }}>CSV一括取込</h2><p style={{ margin: 0, fontSize: 11, color: "#667085" }}>会社名,業界,URL,フォームURL,電話,担当者,優先度,自社担当,出所,メモ</p><textarea name="csv" rows={7} style={input} placeholder="株式会社サンプル,IT,https://example.com,..." /><input name="source_list" placeholder="共通の出所名" style={input} /><button style={btn}>CSV取込</button></form>; }
