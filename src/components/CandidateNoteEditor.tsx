...
// 人材詳細のインライン編集欄。#276③ / #347
//   field を指定して「メール原文（note）」または「人材詳細（detail_note）」を編集・保存する。
//   ENGERフリーランス経由で登録された人材は値が空でも常設の入力欄として置く。
import { useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { toast } from "@/components/toast";
import { updateCandidateById } from "@/lib/actions";

export function CandidateNoteEditor({ candidateNo, initial, field = "note", label = "メール原文", placeholder }: {
  candidateNo: number;
  initial: string;
  field?: "note" | "detail_note";
  label?: string;
  placeholder?: string;
}) {
  ...
  const save = () => {
    start(async () => {
      const r = await updateCandidateById(candidateNo, { [field]: val } as any);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 6, padding: "9px 0", fontSize: 13 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        <textarea
          ...
          rows={3}
          placeholder={placeholder ?? `${label}を入力（保存でこの人材の${label}に反映されます）`}
