...
// KGIダッシュボード：月間売上目標＋人員配分（インサイド/アウトサイド）の手動入力 ＋「AIで週次/日次KPIを計算」ボタン。
...
export function KgiPlanControls({ month, initialTarget, initialInside, initialOutside, hasPlan, canEdit }: {
  ...
  const compute = () => {
    ...
    setBusy(true); setMsg("AIが売上目標と人員配分から逆算しています…");
    (async () => {
      try {
        const r = await computeKgiPlan({ month });
        if (!r.ok) { setMsg(`計算失敗: ${r.error}`); return; }
        setMsg(r.usedAI ? "✓ AIが売上目標・人員配分から週次/日次KPIを割り振りました。" : "✓ 逆算しました（AI未使用・既定の転換率）。");
        router.refresh();
      } catch (e) {
  ...
  const inputStyle: React.CSSProperties = { width: 100, fontSize: 18, fontWeight: 800, padding: "6px 10px", borderRadius: 8, border: "1px solid var(--color-border-strong)", textAlign: "right", background: "var(--color-surface)", color: "var(--color-ink)" };
  const labelWrap: React.CSSProperties = { display: "flex", flexDirection: "column", gap: 4 };
  const labelText: React.CSSProperties = { fontSize: 11, color: "var(--color-ink-4)", fontWeight: 700 };

  return (
    <div className="card" style={{ padding: "14px 16px", display: "flex", flexWrap: "wrap", alignItems: "flex-end", gap: 14 }}>
      <label style={labelWrap}>
        <span style={labelText}>月間売上目標（手動）</span>
      ...
      <label style={labelWrap}>
        <span style={labelText}>インサイド（名）</span>
        <input type="number" min={0} value={inside} disabled={!canEdit || pending}
          onChange={(e) => setInside(e.target.value)} placeholder="例：3" style={inputStyle} />
      </label>
      <label style={labelWrap}>
        <span style={labelText}>アウトサイド（名）</span>
        <input type="number" min={0} value={outside} disabled={!canEdit || pending}
          onChange={(e) => setOutside(e.target.value)} placeholder="例：5" style={inputStyle} />
      </label>
      {canEdit && (
        <>
          <button type="button" className="btn" disabled={pending} onClick={saveTarget}>目標・人員を保存</button>
          <button type="button" className="btn brand" disabled={busy || pending} onClick={compute}
            title="売上目標と人員配分から、必要な提案数・面談数・稼働人数・打ち合わせ数をAIが逆算し、週次/日次に割り振ります。">
