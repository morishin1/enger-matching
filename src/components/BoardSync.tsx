...
import { syncBoardInvoices, boardConnectionTest, autoLinkBoardProjects, boardInvoiceDebug } from "@/app/billing/board-actions";
...
export function BoardSync({ period, lastSyncedAt }: { period: string; lastSyncedAt?: string | null }) {
  ...
  const invDebug = () => start(async () => {
    setMsg(null);
    setDebug("請求データ取得中…");
    const r = await boardInvoiceDebug();
    if (!r.ok) { setDebug(null); setMsg({ ok: false, text: r.error ?? "請求データ取得に失敗しました" }); return; }
    setDebug(JSON.stringify({ keys: r.keys, sample: r.sample }, null, 2));
  });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-end" }}>
      <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
        ...
        <button type="button" className="btn ghost btn-xs" disabled={pending} onClick={invDebug} title="board 請求データの実フィールド名と抽出結果を確認（同期できない原因の特定用）">請求データ診断</button>
