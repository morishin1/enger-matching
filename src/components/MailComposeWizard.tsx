...
export function MailComposeWizard({
  ...
  const handleNext = () => {
    if (!validate()) return;
    // Generate tokens once when entering the confirm step — save will reuse these exact tokens
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (res.existed) {
          // Pre-generated tokens are not in DB for duplicates — clear them
          setJobToken(null);
          setCandToken(null);
          setMsg("既に提案済みです");
        } else {
        ...
        <>
          <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
            <MailPreviewCard
              ...
              body={clientBody} origMailUrl={gmailMessageUrl(job.source_mail_url) || null}
              proposer={proposer} buttonHtml={jobButtonHtml}
            />
            <MailPreviewCard
              title="人材側メール" dotColor="#3b82f6"
              body={candBody} origMailUrl={gmailMessageUrl(cand.source_mail_url) || null}
